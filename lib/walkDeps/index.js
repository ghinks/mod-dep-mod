...
const isCircularDependency = exports.isCircularDependency = ({ ancestry, dependency }) => {
    ...
    if (regDeps) {
      ...
      if (__depends.length > 0) {
        ...
        const newDeps = __depends.map(function (dependency) {
          return { dependency, results: task.results[task.dependency.module], q: task.q, __depth: task.__depth + 1, ancestry: [...task.ancestry, task.dependency.module] };
        });
...
const walkDeps = (() => {
  var _ref2 = (0, _asyncToGenerator3.default)(function* (modules, dependsFile, nodeEnv, done) {
    ...
    q.drain = function () {
      ...
      const matches = modules.reduce(function (acc, moduleToFind) {
        const found = (0, _findNamedModule2.default)(results, moduleToFind, undefined);
        return [...acc, ...found];
      }, []);
    ...
    collatedDeps.forEach(function (d) {
      return q.push({ dependency: d, results, q, __depth: 1, ancestry: [] }, function () {
        return null;
      });
    });
  });
